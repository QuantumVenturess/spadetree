...
class Choice(models.Model):
    ...
    def to_json(self):
        dictionary = {
            ...
            'date'     : nsdate_format(self.date) if self.date else None,
            'date_completed': nsdate_format(
                self.date_completed) if self.date_completed else None,
            'day'         : self.day.to_json() if self.day else None,
            'denied'      : 1 if self.denied else 0,
            'hour'        : self.hour.to_json() if self.hour else None,
            'id'          : self.pk,
